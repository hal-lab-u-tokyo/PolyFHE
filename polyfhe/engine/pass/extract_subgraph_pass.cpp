...
void SortSubgraphNodes(
    ...
    std::vector<std::shared_ptr<polyfhe::core::Node>> sorted_subgraph;
    std::map<std::shared_ptr<polyfhe::core::Node>, bool> sorted;
...
std::optional<std::vector<std::shared_ptr<polyfhe::core::Node>>>
GetSortedSubgraph(std::shared_ptr<polyfhe::core::Node> node,
...
void SetSharedMemOffset(polyfhe::core::SubGraph& subgraph,
...
bool ExtractSubgraphPass::run_on_graph(
    ...
    while (!stack.empty()) {
        ...
        if (has_visited_subnodes) {
            ...
            graph->add_subgraph(
                std::make_shared<polyfhe::core::SubGraph>(subgraph));
            idx_subgraph += 1;
        }
