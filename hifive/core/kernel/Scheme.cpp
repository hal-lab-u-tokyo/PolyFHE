...
complex<double> Scheme::decryptSingle(SecretKey& secretKey,
                                      Ciphertext& cipher) {
    Plaintext msg = decryptMsg(secretKey, cipher);
...
void Scheme::multAndEqual(Ciphertext& cipher1, Ciphertext& cipher2) {
    ...
    context.add(axbx1, cipher1.ax, cipher1.bx,
                cipher1.l); // ax1 + bx1 mod P, 0 mod Q
    context.add(axbx2, cipher2.ax, cipher2.bx,
                cipher2.l); // ax2 + bx2 mod P, 0 mod Q

    context.mulAndEqual(axbx1, axbx2,
                        cipher1.l); // (ax1 + bx1) * (ax2 + bx2) mod P, 0 mod Q

    context.mul(bxbx, cipher1.bx, cipher2.bx,
                cipher1.l); // bx1 * bx2 mod P, 0 mod Q
    context.mul(axax, cipher1.ax, cipher2.ax,
                cipher1.l); // ax1 * ax2 mod P, 0 mod Q
    context.raiseAndEqual(
        axax, cipher1.l); // ax1 * ax2 mod P, ax1 * ax2 + e * P mod Q

    Key key = keyMap.at(
        MULTIPLICATION); // kbx - kax * sx = (sxsx * Q + ex mod P, ex mod Q)
...
Ciphertext Scheme::square(Ciphertext& cipher) {
    ...
    context.add(axbx, cipher.ax, cipher.bx,
                cipher.l); // ax1 + bx1 mod P, 0 mod Q

    context.squareAndEqual(
        axbx, cipher.l); // (ax1 + bx1) * (ax2 + bx2) mod P, 0 mod Q
...
Ciphertext Scheme::multByConstVec(Ciphertext& cipher, complex<double>* cnstVec,
                                  long slots) {
    uint64_t* ax = new uint64_t[cipher.l << context.logN]();
    uint64_t* bx = new uint64_t[cipher.l << context.logN]();
    uint64_t* mx = new uint64_t[cipher.l << context.logN]();
