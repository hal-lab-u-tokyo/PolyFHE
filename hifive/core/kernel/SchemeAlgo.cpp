...
complex<double>* SchemeAlgo::decryptSingleArray(SecretKey& secretKey,
                                                Ciphertext* ciphers,
                                                long size) {
...
Ciphertext* SchemeAlgo::powerOf2Extended(Ciphertext& cipher,
...
Ciphertext* SchemeAlgo::multVec(Ciphertext* ciphers1, Ciphertext* ciphers2,
                                const long size) {
    Ciphertext* res = new Ciphertext[size];
    for (long i = 0; i < size; ++i) {
...
void SchemeAlgo::multAndEqualVec(Ciphertext* ciphers1, Ciphertext* ciphers2,
                                 const long size) {
    for (long i = 0; i < size; ++i) {
...
Ciphertext* SchemeAlgo::multAndModSwitchVec(Ciphertext* ciphers1,
                                            Ciphertext* ciphers2,
                                            const long size) {
    Ciphertext* res = new Ciphertext[size];
    for (long i = 0; i < size; ++i) {
...
void SchemeAlgo::multModSwitchAndEqualVec(Ciphertext* ciphers1,
                                          Ciphertext* ciphers2,
                                          const long size) {
    for (long i = 0; i < size; ++i) {
        scheme.multAndEqual(ciphers1[i], ciphers2[i]);
...
Ciphertext SchemeAlgo::innerProd(Ciphertext* ciphers1, Ciphertext* ciphers2,
                                 const long size) {
...
Ciphertext SchemeAlgo::function(Ciphertext& cipher, string& funcName,
                                const long degree) {
    Ciphertext* cpows = powerExtended(cipher, degree);
...
Ciphertext* SchemeAlgo::functionExtended(Ciphertext& cipher, string& funcName,
                                         const long degree) {
    Ciphertext* cpows = powerExtended(cipher, degree);
    ...
    for (long len = 2; len <= size; len <<= 1) {
        ...
        for (long i = 0; i < size; i += len) {
            for (long j = 0; j < len / 2; ++j) {
                ...
                scheme.multByMonomialAndEqual(ciphers[i + j + len / 2],
                                              shift * j);
