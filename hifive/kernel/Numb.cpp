...
void negate(uint64_t &r, uint64_t a) { r = -a; }
...
void modBarrett(uint64_t &r, unsigned __int128 a, uint64_t m, uint64_t mr,
                long twok) {
    uint64_t atop, abot;
...
uint64_t inv(uint64_t x) { return pow(x, static_cast<uint64_t>(-1)); }
