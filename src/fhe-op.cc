...
FHEOp NewHAdd() { return {"HAdd", {-1, -1, -1, -1}, {-1, -1}}; }

FHEOp NewHMult() { return {"HMult", {-1, -1, -1, -1}, {-1, -1}}; }
...
void update_children(GraphFHE &g, GraphFHE::vertex_descriptor v, int out1,
                     int out2) {
...
void lower_fhe_to_poly(GraphFHE &g_fhe, GraphPoly &g_poly) {
    ...
    for (auto it = vertex_range.first; it != vertex_range.second; ++it) {
        ...
        if (g_fhe[v].name == "HAdd") {
            ...
            // Lower HAdd to Poly
            auto [out1, out2] =
                lower_hadd(g_poly, g_fhe[v].inputs[0], g_fhe[v].inputs[1],
                           g_fhe[v].inputs[2], g_fhe[v].inputs[3]);
            ...
            // Lower HMult to Poly
            auto [out1, out2] =
                lower_hmult(g_poly, g_fhe[v].inputs[0], g_fhe[v].inputs[1],
                            g_fhe[v].inputs[2], g_fhe[v].inputs[3]);
