...
__global__ void BConv_general_part(
    Params *params, uint64_t *edge_MultConst_4_4_BConv_5_0,
    uint64_t *edge_BConv_5_0_NTTPhase1_6_0, const uint64_t *qiHat_mod_pj,
    uint64_t ibase_size, uint64_t obase_start, uint64_t obase_size,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         ...
         tid += blockDim.x * gridDim.x) {
        const size_t n_idx = unroll_number * (tid / obase_size);
        const size_t l_idx = tid % obase_size + obase_start;
        const size_t l_out_idx =
            l_idx + ((l_idx >= startPartIdx) ? size_PartQl : 0);
        uint64_t res1, res2;
        // BConv_5
...
__global__ void MultKeyAccum_8_opt(
    Params *params, int start_limb_original, int end_limb_original,
    int exclude_start, int exclude_end,
    uint64_t *edge_NTTPhase1_24_0_NTTPhase2_25_0, const uint64_t *twiddles,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < (params->N / 8 * (end_limb_original - start_limb_original));
         tid += blockDim.x * gridDim.x) {
        const size_t l_idx = tid / (params->N / 8) + start_limb_original;
        size_t n_idx = tid % (params->N / 8);
        size_t t = params->n2 / 2;
        size_t m_idx = n_idx / (t / 4);
        size_t t_idx = n_idx % (t / 4);
        const size_t n_init = 2 * m_idx * t + t_idx;
        // NTTPhase2
        for (int beta_idx = 0; beta_idx < 5; beta_idx++) {
            ...
            if (l_idx >= (beta_idx + 1) * 6 || l_idx < beta_idx * 6) {
                d_poly_fnwt_phase2_debug2(
                    params, in_list[beta_idx], shared, reg, twiddles,
                    twiddles_shoup, modulus, end_limb_original,
                    start_limb_original, l_idx, twr_idx2, n_init, tid);
                uint64_t *out_ptr = in_list[beta_idx] + l_idx * params->N;
#pragma unroll
                for (size_t j = 0; j < 8; j++) {
                    *(out_ptr + n_init + params->n2 / 8 * j) = reg[j];
                }
...
void entry_kernel(Params *params_d, Params *params_h, PhantomContext &context,
    ...
    if (if_benchmark) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (if_accum_opt) {
                const int obase_size = 30;
                const int ibase_size = 6;
                {
                    const size_t beta_idx = 4;
                    const size_t startPartIdx = params_h->alpha * beta_idx;
                    const size_t size_PartQl =
                        (beta_idx == beta - 1)
                            ? (params_h->L - params_h->alpha * (beta - 1))
                            : params_h->alpha;
                    std::cout << "beta_idx : " << beta_idx
                              << ", startPartIdx : " << startPartIdx
                              << ", size_PartQl : " << size_PartQl << std::endl;
                    auto &bconv_pre =
                        drns_tool
                    ...
                    /*
                    BConv_general<<<
                        params_h->N * obase_size / 128 / unroll_factor, 128,
                        obase_size * ibase_size * sizeof(uint64_t)>>>(
                        params_d, edge_MultConst_4_4_BConv_5_0_d,
                        edge_BConv_5_0_NTTPhase1_6_0_d, bconv_pre.QHatModp(), 6,
                        30, startPartIdx, size_PartQl,
                        params_h->ntt_tables->twiddle(),
                        params_h->ntt_tables->twiddle_shoup(),
                        params_h->ntt_tables->modulus());
                    */
                    // const int n_divide = 36 / size_PartQl;
                    const int limb_per = size_PartQl;
                    int n_divide_ = 30 / size_PartQl;
                    std::cout << "n_divide_ : " << n_divide_ << std::endl;
                    for (int iter = 0; iter < n_divide_; iter++) {
                        BConv_general_part<<<params_h->N * obase_size / 128 /
                            ...
                            bconv_pre.QHatModp(), 6, iter * limb_per, limb_per,
                            startPartIdx, size_PartQl,
                            params_h->ntt_tables->twiddle(),
                            params_h->ntt_tables->twiddle_shoup(),
                            params_h->ntt_tables->modulus());
                    ...
                    n_divide_ = 6;
                    const int limb_per2 = 36 / n_divide_;
                    for (int iter = 0; iter < n_divide_; iter++) {
                        int start_li = iter * limb_per2;
                        int end_li = (iter + 1) * limb_per2;
                        NTTPhase1_general<<<
                            4096, (params_h->n1 / 8) * params_h->pad,
                            (params_h->n1 + params_h->pad + 1) * params_h->pad *
                                sizeof(uint64_t)>>>(
                            params_d, start_li, end_li, 0, 36, 24, 30,
                            edge_BConv_5_0_NTTPhase1_6_0_d,
                ...
                uint64_t **d_in_list;
                checkCudaErrors(
                    cudaMalloc((void **) &d_in_list, sizeof(uint64_t *) * 5));
                checkCudaErrors(cudaMemcpy(d_in_list, in_list,
                                           sizeof(uint64_t *) * 5,
                                           cudaMemcpyHostToDevice));

                MultKeyAccum_8_opt<<<4096, 128,
                                     params_h->per_thread_ntt_size * 128 *
                                         sizeof(uint64_t)>>>(
                    params_d, 0, 36, 0, 6, edge_NTTPhase1_24_0_NTTPhase2_25_0_d,
                    params_h->ntt_tables->twiddle(),
                    params_h->ntt_tables->twiddle_shoup(),
