...
__global__ void MultKeyAccum_8_opt(
    ...
    extern __shared__ uint64_t shared[];
    uint64_t reg[8];
    int n_limb_per_block = (end_limb_original - start_limb_original) / n_blocks;
    ...
    for (int i_block = 0; i_block < n_blocks; i_block++) {
        ...
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            // NTTPhase2
            {
                size_t twr_idx2 =
                    (l_idx >=
                             start_limb_original + end_limb_original - params->K
                         ? params->KL -
                ...
                if (l_idx >= exclude_end || l_idx < exclude_start) {
                    ...
                    d_poly_fnwt_phase2_debug(
                        ...
                        twiddles, twiddles_shoup, modulus, end_limb, start_limb,
                        l_idx, twr_idx2, &n_init_, tid);
                    uint64_t *out_ptr = edge_NTTPhase2_25_0_MultKeyAccum_8_0 +
                                        l_idx * params->N;
#pragma unroll
                    for (size_t j = 0; j < 8; j++) {
