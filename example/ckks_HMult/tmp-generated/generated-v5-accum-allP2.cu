...
__global__ void MultKeyAccum_8_opt(
    ...
    extern __shared__ uint64_t shared[];
    uint64_t reg[8];
    // int n_limb_per_block = (end_limb_original - start_limb_original) /
    // n_blocks;
    ...
    //     int start_limb = start_limb_original + i_block * n_limb_per_block;
    //     int end_limb = start_limb + n_limb_per_block;
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (int beta_idx = 0; beta_idx < 5; beta_idx++) {
            ...
            if (l_idx >= (beta_idx + 1) * 6 || l_idx < beta_idx * 6) {
                uint64_t n_init_;
                d_poly_fnwt_phase2_debug(
                    params, in_list[beta_idx], shared + 128 * 40, reg, twiddles,
                    twiddles_shoup, modulus, end_limb_original,
                    start_limb_original, l_idx, twr_idx2, &n_init_, tid);
                uint64_t *out_ptr = in_list[beta_idx] + l_idx * params->N;
#pragma unroll
                for (size_t j = 0; j < 8; j++) {
                    // *(out_ptr + n_init_ + params->n2 / 8 * j) = reg[j];
                    shared[threadIdx.x * 40 + beta_idx * 8 + j] = reg[j];
                }
            } else {
                for (size_t j = 0; j < 8; j++) {
                    shared[threadIdx.x * 40 + beta_idx * 8 + j] =
...
#pragma unroll
            for (size_t j = 0; j < 8; j++) {
                MulKeyAccumOp_opt(params, edge_MultKeyAccum_8_0_iNTTPhase2_12_0,
                                  ...
                                  relin_keys, 5, idx, l_idx,
                                  shared + threadIdx.x * 40, j);
                idx += params->n2 / 8;
            }
        }
    }
    ...
    if (if_benchmark) {
        ...
        for (int i = 0; i < 100; i++) {
             ...
             */
            {
                ...
                auto &obase = bconv_pre.obase();
                constexpr int unroll_factor = 2;
                BConv_14<<<params_h->N * obase.size() / 128 / unroll_factor,
                           128,
                           obase.size() * ibase.size() * sizeof(uint64_t)>>>(
                    params_d, edge_MultConst_4_3_BConv_14_0_d,
                    edge_BConv_14_0_NTTPhase1_15_0_d, bconv_pre.QHatModp(),
             ...
             */
            {
                ...
                auto &obase = bconv_pre.obase();
                constexpr int unroll_factor = 2;
                BConv_17<<<params_h->N * obase.size() / 128 / unroll_factor,
                           128,
                           obase.size() * ibase.size() * sizeof(uint64_t)>>>(
                    params_d, edge_MultConst_4_2_BConv_17_0_d,
                    edge_BConv_17_0_NTTPhase1_18_0_d, bconv_pre.QHatModp(),
             ...
             */
            {
                ...
                auto &obase = bconv_pre.obase();
                constexpr int unroll_factor = 2;
                BConv_20<<<params_h->N * obase.size() / 128 / unroll_factor,
                           128,
                           obase.size() * ibase.size() * sizeof(uint64_t)>>>(
                    params_d, edge_MultConst_4_1_BConv_20_0_d,
                    edge_BConv_20_0_NTTPhase1_21_0_d, bconv_pre.QHatModp(),
            ...
            NTTPhase1_general<<<4096, (params_h->n1 / 8) * params_h->pad,
                                (params_h->n1 + params_h->pad + 1) *
                                    params_h->pad * sizeof(uint64_t)>>>(
                params_d, 0, 36, 0, 36, 6, 12, edge_BConv_20_0_NTTPhase1_21_0_d,
                edge_NTTPhase1_21_0_NTTPhase2_22_0_d,
                params_h->ntt_tables->twiddle(),
                params_h->ntt_tables->twiddle_shoup(),
                params_h->ntt_tables->modulus());
...

            {
                ...
                auto &obase = bconv_pre.obase();
                constexpr int unroll_factor = 2;
                BConv_23<<<params_h->N * obase.size() / 128 / unroll_factor,
                           128,
                           obase.size() * ibase.size() * sizeof(uint64_t)>>>(
                    params_d, edge_MultConst_4_0_BConv_23_0_d,
                    edge_BConv_23_0_NTTPhase1_24_0_d, bconv_pre.QHatModp(),
            ...
            NTTPhase1_general<<<4096, 128,
                                (params_h->n1 + params_h->pad + 1) *
                                    params_h->pad * sizeof(uint64_t)>>>(
                params_d, 0, 36, 0, 36, 0, 6, edge_BConv_23_0_NTTPhase1_24_0_d,
                edge_NTTPhase1_24_0_NTTPhase2_25_0_d,
                params_h->ntt_tables->twiddle(),
                params_h->ntt_tables->twiddle_shoup(),
                params_h->ntt_tables->modulus());
            ...
            MultKeyAccum_8_opt<<<4096, 128,
                ...
                edge_NTTPhase2_7_0_MultKeyAccum_8_4_d,
                edge_MultKeyAccum_8_0_iNTTPhase2_12_0_d,
                edge_MultKeyAccum_8_1_iNTTPhase2_9_0_d, relin_keys, n_divide);
            checkCudaErrors(cudaDeviceSynchronize());
