...
__global__ void BConv_general_part(
    Params *params, int beta_idx, uint64_t **in_list, uint64_t **out_list,
    ...
    //     (beta_idx == beta - 1) ? (params->L - alpha * (beta - 1)) :
    //     alpha;
                ...
                for (int iter = 0; iter < n_divide_; iter++) {
                    ...
                    for (int beta_idx = 0; beta_idx < beta; beta_idx++) {
                        ...
                        BConv_general_part<<<params_h->N * limb_per / 128 /
                                                 unroll_factor,
                                             128>>>(
                            params_d, beta_idx, d_bconv_in_list,
                            d_bconv_out_list, d_qhat_modp_list, 6, start_li,
                            limb_per, params_h->alpha, beta,
                            params_h->ntt_tables->twiddle(),
                            params_h->ntt_tables->twiddle_shoup(),
                            params_h->ntt_tables->modulus());
                    }
                    */
                    BConv_general_part_allbeta<<<
                        params_h->N * limb_per / 128 / unroll_factor, 128>>>(
                        params_d, d_bconv_in_list, d_bconv_out_list,
                        d_qhat_modp_list, 6, start_li, limb_per,
