        ...
        // NTTPhase2
        {
            size_t twr_idx2 =
                (l_idx >= start_limb_original + end_limb_original - params->K
                     ...
                     : l_idx);
            if (l_idx >= exclude_end || l_idx < exclude_start) {
                uint64_t n_init_;
                d_poly_fnwt_phase2_debug(
                    params, edge_NTTPhase1_24_0_NTTPhase2_25_0, shared, reg,
                    twiddles, twiddles_shoup, modulus, end_limb_original,
                    start_limb_original, l_idx, twr_idx2, &n_init_, tid);
                uint64_t *out_ptr =
                    edge_NTTPhase2_25_0_MultKeyAccum_8_0 + l_idx * params->N;
#pragma unroll
                for (size_t j = 0; j < 8; j++) {
