...
__global__ void BConv_general_part(
    ...
    uint64_t ibase_size, uint64_t obase_start, uint64_t obase_start_in30,
    uint64_t obase_size, size_t startPartIdx, size_t size_PartQl,
...
__global__ void NTTPhase1_general_per(
    Params *params, int start_limb, int end_limb, int start_limb_original,
    int end_limb_original, uint64_t *edge_BConv_23_0_NTTPhase1_24_0,
    uint64_t *edge_NTTPhase1_24_0_NTTPhase2_25_0, const uint64_t *twiddles,
    const uint64_t *twiddles_shoup, const DModulus *modulus) {
    extern __shared__ uint64_t shared[];
    uint64_t reg[8];
            ...
            if (if_accum_opt) {
                ...
                const int ibase_size = 6;
                {
                    ...
                    */
                    ...
                    int n_divide_ = 36 / size_PartQl;
                    int larger_than_startPartIdx = 0;
                    for (int iter = 0; iter < n_divide_; iter++) {
                        if (iter * limb_per == startPartIdx) {
                        ...
                        BConv_general_part<<<params_h->N * obase_size / 128 /
                            ...
                            bconv_pre.QHatModp(), 6, iter * limb_per,
